...
class CodeService extends Service {
  async import(actId, ticketId, ticketCodes) {
    ...
    if (!ticketCodes || ticketCodes.length < 1) {
      this.log('error', 'ticket codes is not right', {
        actId,
        ticketId,
        ticketCodes,
      });
    ...
    for (const ticketCode of ticketCodes) {
      ...
      let codeInfo = await codeModel.getInfoByCodeAndTicket(
        ticketCode,
        ticketId,
      );
      if (codeInfo) {
    ...
    if (!filterList || filterList.length < 1) {
      this.log('error', 'has alreay import', {
        actId,
        ticketCodes,
        filterList,
      });
    ...
    if (!mongoRet) {
      this.log('error', 'insert mongodb error, pls retry later', {
        mongoRet,
        rowList,
      });
    }
    ...
    rowList.forEach(async rowInfo => {
      let setRet = await cache.set(
        CODE_TICKET_MAPPING.replace('{code}', rowInfo['code']),
        rowInfo['ticket_id'],
        CACHE_TIME_SEC,
      );
      if (!setRet) {
      ...
      let ticketCacheRet = await ticketService.cacheDetailById(
        rowInfo['ticket_id'],
      );
      if (!ticketCacheRet) {
  ...
  async getUserJoinCode(actId) {
    let ticketCode = await cache.get(
      ACT_USER_JOIN_RESULT.replace('{actId}', actId).replace(
        '{userId}',
        this.ctx.userId,
      ),
    );
  ...
  async getOneCode(actId) {
    ...
    if (!setRet) {
      ...
      this.log('error', 'set redis error, need reget this code', {
        setRet,
        ticketCode,
      });
      return -1;
