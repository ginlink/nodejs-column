...
module.exports = function (id, startTime = '18:12:13', intervals = oneDaySecs) {
  ...
  const currentZeroSecs =
    date.getHours() * oneHourSecs +
    date.getMinutes() * oneMinSecs +
    date.getSeconds();
  ...
  console.log(
    `系统将在 ${nextTimes} 秒后打印首次内存快照，请在首次快照后请求内存泄漏接口`,
  );
  /// 定时打印内存快照
  setTimeout(() => {
    dumpFile(id);
...
function dumpFile(id = 'test') {
  ...
  const fileName = `./log/heapdump_${id}_${current.getFullYear()}${
    current.getMonth() + 1
