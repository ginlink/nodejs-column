...
class ActService extends Service {
  async getList(page = 0) {
    ...
    cache
      .set(ACT_LIST_CACHE_KEY.replace('{page}', page), actList, CACHE_TIME_SEC)
      .then();
  ...
  async cacheList(cacheSize = 5) {
    ...
    for (let i = 0; i < cacheSize; i++) {
      ...
      let ret = await cache.set(
        ACT_LIST_CACHE_KEY.replace('{page}', i),
        actList,
        CACHE_TIME_SEC,
      );
      if (!ret) {
  ...
  async cacheDetail(actInfo) {
    ...
    let ret = await cache.set(
      ACT_DETAIL_CACHE_KEY.replace('{actId}', actInfo['_id']),
      actInfo,
      CACHE_TIME_SEC,
      true,
    );
